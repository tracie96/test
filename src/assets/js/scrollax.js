...
(function(factory) {
  ...
  var Scrollax = function(frame, options, callbackMap) {
     ...
     */
    function load() {
      $parents = frameIsWindow
        ? $doc.find(parentSelector)
        : $frame.find(parentSelector);
      // Reset parents
      parents.length = 0;
    ...
    function scrollHandler() {
      ...
      for (var i = 0; i < l; i++) {
        ...
        if (
          pos[isHorizontal ? "right" : "bottom"] < 0 ||
          pos[isHorizontal ? "left" : "top"] >
            offset[isHorizontal ? "width" : "height"]
        ) {
          continue;
        }
      ...
      for (property in properties) {
        ...
        if ($.inArray(property, transforms) !== -1) {
          ...
          element.style[property] =
            property === "opacity"
              ? within(ratio < 0 ? 1 + value : 1 - value, 0, 1)
              : value;
        }
      }
    ...
    return this.each(function(i, element) {
      ...
      if (!plugin && !method) {
        ...
        plugin = $.data(
          element,
          lownamespace,
          new Scrollax(element, options, callbackMap).init()
        );
